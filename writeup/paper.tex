...
\author{
\IEEEauthorblockN{Dominic Tanzillo}
...
Low-Earth orbit (LEO) is approaching a critical inflection point. With over 30,000 tracked objects and millions of untracked fragments, the self-sustaining cascading collision process predicted by Kessler \& Cour-Palais~\cite{kessler1978} is no longer theoretical. The CRASH Clock metric~\cite{thiele2025}---which estimates the expected time between collisions based on population density, relative velocities, and collision cross-sections---has decreased from approximately 164 days in 2018 to just 5.5 days in 2025, primarily driven by mega-constellations like Starlink.
...
The ESA Kelvins Collision Avoidance Challenge~\cite{uriot2021} established the benchmark for ML approaches to CDM-based risk classification. Competition winners~\cite{stevenson2020,siew2020} demonstrated that gradient-boosted trees with carefully engineered features could achieve strong performance. However, several limitations persist:
...
\item \textbf{Lack of uncertainty quantification}: All competition entries produced point predictions. NASA CARA has explicitly identified the absence of calibrated uncertainty estimates as a barrier to operational adoption of ML methods~\cite{hejduk2023}.
...
\item \textbf{Density augmentation}: We integrate population-level orbital density features derived from the CRASH Clock framework~\cite{thiele2025}, providing shell-level collision rate context as static enrichment to the temporal model.
...
The ESA Kelvins Challenge~\cite{uriot2021} provided the foundational dataset: 162,634 CDMs across 13,154 conjunction events with 103 numerical features. Key approaches:

\begin{itemize}
\item \textbf{Stevenson et al.}~\cite{stevenson2020} achieved top performance using gradient-boosted trees with temporal aggregation features (miss distance trends, covariance evolution). Their key insight: the \emph{trajectory} of CDM updates matters more than any single update.
\item \textbf{Siew et al.}~\cite{siew2020} explored neural network approaches but found classical methods competitive, noting the challenge of learning from small event counts with extreme class imbalance ($\sim$3.4\% positive rate).
\item \textbf{Acciarini et al.}~\cite{acciarini2021} applied LSTM-based sequence models to CDM time series, demonstrating the feasibility of temporal deep learning but without matching boosted tree performance.
\end{itemize}

\subsection{Temporal Fusion Transformers}
\label{sec:tft}

Lim et al.~\cite{lim2021} introduced the Temporal Fusion Transformer (TFT) for multi-horizon time series forecasting, featuring:
\begin{itemize}
...
Thiele et al.~\cite{thiele2025} developed the Collision Rate Assessment of Space Hazards (CRASH) Clock, estimating collision frequency as:
\begin{equation}
...
Conformal prediction~\cite{vovk2005,angelopoulos2021} provides distribution-free prediction sets with guaranteed marginal coverage:
\begin{equation}
P(Y \in C(X)) \geq 1 - \alpha
\label{eq:conformal}
\end{equation}

Split conformal prediction~\cite{lei2018} is particularly suitable for deployment: calibrate on a held-out set, compute nonconformity score quantiles, and construct prediction sets at test time with no distributional assumptions.
...
The ESA Kelvins Collision Avoidance Challenge dataset~\cite{uriot2021}:
\begin{itemize}
...
We classify at $5 \times 10^{-4}$, NASA CARA's ``red'' threshold where operators begin planning collision avoidance maneuvers~\cite{hejduk2023}. This gives 19.1\% positive rate (97 of 506 pairs exceeded $5 \times 10^{-4}$), providing sufficient class balance for meaningful ML.
...
\bibitem{acciarini2021}
G.~Acciarini et~al., ``Spacecraft collision risk assessment with probabilistic programming,'' in \emph{NeurIPS ML4PS Workshop}, 2021.
...
L.~Grinsztajn et~al., ``Why do tree-based models still outperform deep learning on tabular data?'' in \emph{Advances in Neural Information Processing Systems (NeurIPS)}, 2022.

\bibitem{hejduk2023}
M.~Hejduk, ``AI/ML research for conjunction assessment,'' NASA CARA Technical Report, 2023.

\bibitem{kessler1978}
D.~J. Kessler and B.~G. Cour-Palais, ``Collision frequency of artificial satellites: The creation of a debris belt,'' \emph{Journal of Geophysical Research}, 1978.

\bibitem{lei2018}
J.~Lei et~al., ``Distribution-free predictive inference for regression,'' \emph{Journal of the American Statistical Association}, 2018.

\bibitem{levit2010}
C.~Levit and W.~Marshall, ``Improved orbit predictions using two-line elements,'' \emph{Advances in Space Research}, 2010.

\bibitem{lim2021}
B.~Lim et~al., ``Temporal fusion transformers for interpretable multi-horizon time series forecasting,'' \emph{International Journal of Forecasting}, 2021.

\bibitem{mcelfresh2023}
D.~McElfresh et~al., ``When do neural nets outperform boosted trees on tabular data?'' in \emph{Advances in Neural Information Processing Systems (NeurIPS)}, 2023.

\bibitem{siew2020}
P.~M. Siew et~al., ``Towards collision avoidance with deep learning,'' Kelvins Challenge Report, 2020.

\bibitem{stevenson2020}
E.~Stevenson et~al., ``Probabilistic machine learning approach for conjunction assessment,'' Kelvins Challenge Winner Report, 2020.

\bibitem{thiele2025}
T.~Thiele et~al., ``The CRASH clock: Collision rate assessment of space hazards,'' in review, 2025.

\bibitem{uriot2021}
T.~Uriot et~al., ``Spacecraft collision avoidance challenge,'' Kelvins/ESA, 2021.

\bibitem{vovk2005}
V.~Vovk et~al., \emph{Algorithmic Learning in a Random World}.\hskip 1em plus 0.5em minus 0.4em\relax Springer, 2005.
