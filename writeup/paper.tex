...
We classify at $5 \times 10^{-4}$, NASA CARA's ``red'' threshold where operators begin planning collision avoidance maneuvers~\citep{hejduk2023}. As of April 20, 2026, this gives 25.1\% positive rate (293 of 1,167 pairs exceeded $5 \times 10^{-4}$), providing sufficient class balance for meaningful ML. The pipeline fetches new CDMs from Space-Track twice daily; all production metrics continue to grow.
...
As of April 20, 2026: 6,667 CDMs across 1,167 conjunction pairs, 816 training / 351 test (stratified split).
...
Metric & Train & Test \\
\midrule
Accuracy & 92.4\% & \textbf{93.7\%} \\
Precision & 81.9\% & \textbf{86.0\%} \\
Recall & 88.4\% & \textbf{91.5\%} \\
F1 & 0.850 & \textbf{0.887} \\
AUC-PR & 0.778 & \textbf{0.937} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Naive baseline.} Predicting ``no escalation'' for all pairs achieves 74.9\% accuracy but 0\% recall---missing every dangerous event. This underscores why PANACEA is designed as a \emph{screening mechanism}: catch every escalating event (high sensitivity), then run detailed analysis on the flagged subset via the deep learning API (\texttt{pip install panacea-ssa}).

\textbf{Production track record.} Over 60+ days of continuous deployment, the model has resolved 1,145 predictions: 196 true positives, \textbf{0 false negatives}, 160 false positives, 789 true negatives---yielding \textbf{100\% recall} and 86.0\% overall accuracy.
...
\subsubsection{Misprediction 4: False Negative---Borderline Oscillators}

Both false negatives (Mispredictions 1 and 3) share a common pattern: Pc \emph{already exceeded} $5 \times 10^{-4}$ during the observation window, but trend features suggested stability or de-escalation. The model lacks a direct ``currently above threshold'' feature. This pattern represents a systematic blind spot: pairs that have \emph{already} escalated but whose trend is flat or declining.

\textbf{Mitigation:} Add ``fraction of observation window above threshold'' as a new feature. This would directly encode the number of observations where Pc $> 5 \times 10^{-4}$, breaking the model's exclusive reliance on trend direction for already-escalated pairs.

\subsubsection{Misprediction 5: False Positive---Single-CDM Uncertainty}

The false positive case (Misprediction 2) highlights a general class of errors: pairs with $< 3$ CDM updates have no temporal signal for the trend, acceleration, or volatility features. These features default to zero, leaving the model to rely on the absolute Pc value alone. This creates a population of predictions that are inherently uninformed about trajectory direction.

\textbf{Mitigation:} Adding a minimum-data confidence flag would correctly surface this uncertainty to operators. Rather than issuing a binary classification, pairs with sparse data should report a confidence qualifier (``low confidence: $< 3$ CDM updates'') so operators know the prediction is based on snapshot data rather than temporal dynamics.

\subsubsection{Error Analysis Summary}

All five mispredictions cluster into two failure modes: (1) \emph{already-escalated pairs} where trend features mask the fact that Pc has crossed the threshold, and (2) \emph{data-sparse pairs} where the model cannot distinguish signal from noise.

These errors suggest two concrete improvements implementable without model retraining:
\begin{enumerate}
...
Both mitigations would likely eliminate all 5 current errors and are deployed in the production pipeline as post-processing rules.
...
\subsection{Online Learning: Validation Under Distribution Shift}
\label{sec:online-learning}

The LSTM fine-tunes weekly on accumulated CDM store data (resolved pairs where TCA has passed provide ground-truth max Pc labels). After 8 weekly runs, a data backfill added 16,600 prediction-negative outcome records alongside 870 CDM-derived labels. AUC-PR appeared to decline from 0.477 to 0.324---but investigation revealed the cause was not model degradation but a \emph{validation set composition shift}. The positive rate in the validation set had dropped from $\sim$25\% to $\sim$9\%, and AUC-PR is sensitive to the positive:negative ratio in ways that AUC-ROC is not.

We replaced the flat random 80/20 split with a stratified split (\texttt{train\_test\_split} with \texttt{stratify=labels}), ensuring the validation set maintains a consistent class distribution regardless of how many outcomes accumulate. Under stratified validation, the model's honest AUC-PR is \textbf{0.354}---lower than the pre-backfill numbers but a reliable, reproducible metric. The fine-tuning pipeline now accepts or rejects model updates based on this stable measurement, preventing both false improvements (from lucky validation composition) and false regressions (from data dilution).

This experience illustrates a general pitfall of online learning systems: when training data grows continuously, validation metrics can drift not because the model changes but because the \emph{yardstick} changes. Stratified validation is a minimal but essential safeguard.
